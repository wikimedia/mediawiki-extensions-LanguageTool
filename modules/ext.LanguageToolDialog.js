...
ve.ui.LanguageToolDialog.prototype.initialize = function () {
	...
	this.$findResults = this.$( '<div>' ).addClass( 've-ui-LanguageToolDialog-findResults' );
	this.fragments = [];
	this.results = 0;
	...
	*/
	var optionsGroup = new OO.ui.ButtonGroupWidget( {
		...
		$findRow = this.$( '<div>' ).addClass( 've-ui-LanguageToolDialog-row' ),
		$replaceRow = this.$( '<div>' ).addClass( 've-ui-LanguageToolDialog-row' );
...
ve.ui.LanguageToolDialog.prototype.getTeardownProcess = function ( data ) {
	return ve.ui.LanguageToolDialog.super.prototype.getTeardownProcess.call( this, data )
		.next( function () {
...
ve.ui.LanguageToolDialog.prototype.renderRangeOfFragments = function ( range ) {
	...
	for ( i = range.start; i < range.end; i++ ) {
		...
		$result = this.$( '<div>' ).addClass( 've-ui-LanguageToolDialog-findResult' );
		top = Infinity;
 ...
 */
ve.ui.LanguageToolDialog.prototype.highlightFocused = function ( scrollIntoView ) {
	var $result, rect, top,
	...
	if ( this.renderedFragments.containsOffset( this.focusedIndex ) ) {
		$result = this.$findResults.children().eq( this.focusedIndex - this.renderedFragments.start )
			.addClass( 've-ui-LanguageToolDialog-findResult-focused' );
